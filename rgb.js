...
const populationsizeR=200;
const populationsizeG=200;
const populationsizeB=200;
...
function newPopulation(){
	populationR=[];
	for(let n=0;n<populationsizeR;n++){
		populationR[n]=[];
		for(let i=0;i<neurosize;i++){
			populationR[n][i]=2-Math.random()*4;
		}
	}
	populationG=[];
	for(let n=0;n<populationsizeG;n++){
		populationG[n]=[];
		for(let i=0;i<neurosize;i++){
	...
	populationB=[];
	for(let n=0;n<populationsizeB;n++){
		populationB[n]=[];
		for(let i=0;i<neurosize;i++){
...
function init(){
	...
	for(let i=0;i<populationR.length;i++){
		aR[i]=[];
		aR[i][0]=hpstartR;							//hp
	...
	for(let i=0;i<populationB.length;i++){
		aB[i]=[];
		aB[i][0]=hpstartB;							//hp
